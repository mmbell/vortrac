...
Dorade::Dorade(const QString &radarname, const float &lat, const float &lon, const QString &filename) 
...
bool Dorade::readVolume()
{
	...
	numSweeps++;
	Sweeps[0].setSweepIndex( numSweeps );
	Sweeps[0].setElevation( ryptr[0].elevation );
	Sweeps[0].setUnambig_range( rptr->unambig_range );
	Sweeps[0].setNyquist_vel( rptr->unambig_vel );
	Sweeps[0].setFirst_ref_gate( cptr->gate_spacing[0] );
	Sweeps[0].setFirst_vel_gate( cptr->gate_spacing[0] );
	Sweeps[0].setRef_gatesp( cptr->gate_spacing[1] - cptr->gate_spacing[0] );
	Sweeps[0].setVel_gatesp( cptr->gate_spacing[1] - cptr->gate_spacing[0] );
	Sweeps[0].setRef_numgates( cptr->total_gates );
	Sweeps[0].setVel_numgates( cptr->total_gates );
	Sweeps[0].setVcp( -999 );
	
	
	for (int i=0; i < getNumRays(); i++) {
		if (i == 0) {
			QDateTime rayTime = getRayTime(i);
			radarDateTime.setTimeSpec(Qt::UTC);
			radarDateTime.setDate(rayTime.date());
			radarDateTime.setTime(rayTime.time());
		}
		
		// The radar time is set above so these aren't used in the Dorade case
		Rays[i].setTime( -999 );
		Rays[i].setDate( -999 );
		
		numRays++;
		Rays[i].setSweepIndex( (numSweeps - 1) );
		Rays[i].setAzimuth( ryptr[i].azimuth );
		Rays[i].setElevation( ryptr[i].elevation );
		Rays[i].setVelResolution( -999 );
		Rays[i].setRayIndex( i );
		Rays[i].setUnambig_range( rptr->unambig_range );
		Rays[i].setNyquist_vel( rptr->unambig_vel );
		Rays[i].setFirst_ref_gate( cptr->gate_spacing[0] );
		Rays[i].setFirst_vel_gate( cptr->gate_spacing[0] );
		Rays[i].setRef_gatesp( cptr->gate_spacing[1] - cptr->gate_spacing[0] );
		Rays[i].setVel_gatesp( cptr->gate_spacing[1] - cptr->gate_spacing[0] );
		Rays[i].setRef_numgates( cptr->total_gates );
		Rays[i].setVel_numgates( cptr->total_gates );
		Rays[i].setVcp( -999 );
		

		Rays[i].setRefData( getReflectivity(i) );
		Rays[i].setVelData( getRadialVelocity(i) );
		Rays[i].setSwData( getSpectrumWidth(i) );
...
/**************************************************/
bool Dorade::sweepread(const char swp_fname[],struct sswb_info *ssptr, struct vold_info *vptr,
