...
void Log::catchLog(const Message& logEntry)
{
    ...
    if(progress!=0) {
        ...
        if (absoluteProgress > 100) absoluteProgress = 100;
    ...
    if((stopLightColor!=AllOff) || (stopLightMessage!=QString())) {
        newStopLightColor(stopLightColor,stopLightMessage);
...
//        }
    ...
    if((stormSignalStatus!=Nothing) || (stormSignalMessage!=QString())) {
        newStormSignalStatus(stormSignalStatus,stormSignalMessage);
